...
%         y_temp(ind_y) = 1;
%     end
%     a_1 = zeros(size(X,2)+1,1); 
%     a_1(1) = 1; a_2 = zeros(size(Theta1,1)+1,1); a_2(1) = 1;
%     a_1(2:end) = X(i,:)';
%     a_2(2:end) = sigmoid(Theta1*a_1);
%     a_3 = sigmoid(Theta2*a_2);
%     J = J+(-y_temp'*log(a_3) - (1-y_temp)'*log(1-a_3));
% end
% J = J/m;
...
reg = (lambda/(2*m))*(sum(sum(Theta1(:,2:end).^2)) + sum(sum(Theta2(:,2:end).^2)));
J = J + reg;
reg1 = zeros(size(Theta1)); reg1(:,2:end) = (lambda/m)*(Theta1(:,2:end));
reg2 = zeros(size(Theta2)); reg2(:,2:end) = (lambda/m)*(Theta2(:,2:end));
